...
앱은 `members`에 `is_active = true`로 등록된 이메일만 문서 읽기, 작성, 수정,
이미지 업로드를 허용합니다. 멤버십 조회는 로그인 이메일과 `members.email`을
직접 비교하므로, 이메일은 소문자로 등록하세요. 첫 owner 계정을 수동 등록한 뒤
로그인하면 `/admin/members`에서 이후 멤버의 Auth user와 `members` row를 함께
생성할 수 있습니다.
