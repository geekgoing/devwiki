...
8. `20260526131913_search_comments_conflicts_tests.sql`
9. `20260602134005_remove_completed_document_state.sql`
...
일곱 번째 migration은 멤버별 문서 상태를 저장하는
`document_member_states` 테이블과 RLS 정책을 추가합니다.
여덟 번째 migration은 문서 본문 검색용 `search_vector`와
`search_documents` RPC, 댓글 해결 상태와 editor/author 댓글 관리 정책을
추가합니다.
아홉 번째 migration은 숙지 완료 상태 컬럼과 관련 인덱스를 제거하고,
즐겨찾기하지 않은 멤버별 문서 상태 row를 정리합니다.
